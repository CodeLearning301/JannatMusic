...
async def admin_cbs(_, query: CallbackQuery):
    ...
    if data == "resume_cb":
        ...
        get = jannatdb.get(query.message.chat.id)
        if not get:
            try:
                await _clear_(query.message.chat.id)
                await pytgcalls.leave_group_call(query.message.chat.id)
...
@app.on_callback_query(filters.regex("jannat_help"))
...
@app.on_callback_query(filters.regex("jannat_home"))
async def home_jannat(_, query: CallbackQuery):
    try:
        await query.answer()
    except:
        pass
